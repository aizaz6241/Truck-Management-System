...
export type FilterType = "today" | "7d" | "30d" | "6m" | "1y" | "3y" | "all" | "date" | "month" | "year";
...
export async function getRevenueStats(filterType: FilterType, dateParam?: string) {
    let startDate = new Date();
    let endDate = new Date();
...
export async function getInvoiceStats(filterType: FilterType, dateParam?: string) {
    let startDate = new Date();
    let endDate = new Date();
...
export async function getContractorStats(filterType: FilterType, dateParam?: string) {
    let startDate = new Date();
    let endDate = new Date();
    ...
    const contractorMap: { [key: string]: { name: string, total: number, received: number, remaining: number } } = {};

    invoices.forEach(inv => {
        const contractorName = inv.contractor.name;
        if (!contractorMap[contractorName]) {
            contractorMap[contractorName] = { name: contractorName, total: 0, received: 0, remaining: 0 };
        }
...
export async function getContractorTimeline(
  ...
  const addToMap = (date: Date, type: "invoiced" | "received", amount: number) => {
...
export async function getDieselAnalytics(filterType: FilterType, dateParam?: string) {
    let startDate = new Date();
    let endDate = new Date();
