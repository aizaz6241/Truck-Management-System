...
export default function DriverAnalytics({ drivers }: DriverAnalyticsProps) {
  ...
  const [selectedDriverId, setSelectedDriverId] = useState<number | "all">(
    "all",
  );
  ...
  // Pie Chart Data: Total Trips by Driver
  const pieData = useMemo(() => {
    return drivers
      .map((d) => ({
        name: d.name,
        value: filterTrips(d.trips).length,
      }))
      .filter((d) => d.value > 0);
  }, [
    drivers,
  ...
  const lineData = useMemo(() => {
    ...
    if (selectedDriverId === "all") {
      // Aggregate all trips
      drivers.forEach((d) => {
        filteredTrips = [...filteredTrips, ...d.trips];
      });
    } else {
      const driver = drivers.find((d) => d.id === selectedDriverId);
      if (driver) {
        filteredTrips = driver.trips;
      }
    }
    ...
    filteredTrips.forEach((trip) => {
      ...
      if (aggregationType === "daily") {
        // Format: YYYY-MM-DD for sorting/grouping
          ...
          >
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setSelectedDriverId(val === "all" ? "all" : Number(val));
              }}
              style={{
                padding: "0.5rem",
                borderRadius: "0.375rem",
