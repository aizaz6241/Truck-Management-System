...
export default function TotalTripsModal({
  isOpen,
  onClose,
  trips,
  dateLabel,
}: TotalTripsModalProps) {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text("Total Trips Report", 14, 22);
    ...
    tripsData.forEach((trip) => {
      ...
      if (!grouped[vNo]) {
        let ownerName = "-";
        if (trip.vehicle.ownership === "Taxi") {
          ownerName =
    ...
    autoTable(doc, {
      ...
      body: tableRows,
      startY: 40,
      headStyles: { fillColor: [59, 130, 246], textColor: 255 }, // Blue for Total/All
      alternateRowStyles: { fillColor: [239, 246, 255] }, // Blue-50
      styles: { fontSize: 10, cellPadding: 4 },
      columnStyles: {
        0: { fontStyle: "bold" }, // Vehicle No
        5: { cellWidth: 60 }, // Wider column for routes
      },
    });

    let fileName = `Total_Trips_${new Date().toISOString().split("T")[0]}.pdf`;

    if (dateRange.startDate && dateRange.endDate) {
      fileName = `Total_Trips_${dateRange.startDate}_${dateRange.endDate}.pdf`;
    } else if (dateLabel) {
      // Sanitize dateLabel for filename
      const safeLabel = dateLabel.replace(/[^a-z0-9]/gi, "_").toLowerCase();
      fileName = `Total_Trips_${safeLabel}.pdf`;
    }
        ...
        >
          ...
          >
            <div>
              ...
              >
                Summary of all vehicle activity (RVT + Taxi).
              </p>
            </div>
            <button
              type="button"
